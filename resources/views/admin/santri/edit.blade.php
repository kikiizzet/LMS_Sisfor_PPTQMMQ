...
<div class="content-wrapper">
    <div class="container-lg mt-4 mb-5">
        <div class="d-flex align-items-center mb-4">
            ...
            <div>
                <h1 class="h3 fw-bold mb-1">Edit Data Santri</h1>
                <p class="text-muted small">Perbarui informasi santri</p>
            </div>
        </div>
                ...
                <form action="{{ route('admin.santri.update', $santri) }}" method="POST">
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="nama_lengkap" class="form-control form-control-minimal" value="{{ old('nama_lengkap', $santri->nama_lengkap) }}" required>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label-custom">No Induk</label>
                            <input type="text" name="no_induk" class="form-control form-control-minimal" value="{{ old('no_induk', $santri->no_induk) }}" required>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label-custom">NISN</label>
                            <input type="text" name="nisn" class="form-control form-control-minimal" value="{{ old('nisn', $santri->nisn) }}">
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="jenis_kelamin" class="form-select form-control-minimal" required>
                                <option value="L" {{ old('jenis_kelamin', $santri->jenis_kelamin) == 'L' ? 'selected' : '' }}>Laki-laki (L)</option>
                                <option value="P" {{ old('jenis_kelamin', $santri->jenis_kelamin) == 'P' ? 'selected' : '' }}>Perempuan (P)</option>
                            </select>
                        </div>
                        <div class="col-md-4">
                            ...
                            <select name="status" class="form-select form-control-minimal" required>
                                ...
                                <option value="Pindah" {{ old('status', $santri->status) == 'Pindah' ? 'selected' : '' }}>Pindah Keluar</option>
                            </select>
                        </div>

                        <div class="col-md-6">
                            <label class="form-label-custom">Tempat Lahir</label>
                            <input type="text" name="tempat_lahir" class="form-control form-control-minimal" value="{{ old('tempat_lahir', $santri->tempat_lahir) }}">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label-custom">Tanggal Lahir</label>
                            <input type="date" name="tanggal_lahir" class="form-control form-control-minimal" value="{{ old('tanggal_lahir', $santri->tanggal_lahir) }}">
                        </div>
                    </div>
                    
                    <hr class="my-4" style="border-color: #e2e8f0;">
                    <div class="text-end">
                        <button type="submit" class="btn btn-primary px-5 py-2" style="border-radius: 12px; font-weight: 600;">Update Data Santri</button>
                    </div>
                </form>
            </div>
        </div>
